/*    There is a bi-directional graph with n vertices, where each vertex is labeled from 0 to n - 1 (inclusive).
      ...
      You want to determine if there is a valid path that exists from the vertex source to the vertex destination.
...
class Solution {
    ...
    private boolean dfs(List<List<Integer>> adjList, boolean[] visited, int current, int destination) {
        ...
        for (int neighbor : adjList.get(current)) {
            if (!visited[neighbor] && dfs(adjList, visited, neighbor, destination)) {
                return true;
            }
        }
